...
const Navigation = () => {
	...
	return (
		...
		<section className={`fixed sm:static left-0 top-0 sm:flex sm:justify-center ${mobileNavOpen ? "bg-accent-light/25 dark:bg-accent-dark/25 backdrop-blur z-20" : ""}`}>
			{mobileNavOpen ? (
			...
			{/* navbar */}
			<nav className={`p-6 pb-4 w-screen h-screen sm:h-auto ${mobileNavOpen ? "flex" : "hidden"} sm:flex flex-col sm:flex-row justify-center items-center gap-6 sm:gap-x-12 border-b-2 border-solid border-transparent sm:border-accent-light sm:dark:border-accent-dark`}>
				{/* array map links */}
				{links.map((link, i) => (
